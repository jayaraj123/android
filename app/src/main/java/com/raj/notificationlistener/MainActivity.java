package com.raj.notificationlistener;
...
import com.raj.notificationlistener.model.Blocklist;
...
public class MainActivity extends AppCompatActivity {
   ...
   private ImageChangeBroadcastReceiver imageChangeBroadcastReceiver;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        FirebaseFirestore.getInstance().collection("Blocklist").document(
                "OG72oO8MaeGKpYdx9sbX").get().addOnCompleteListener( new OnCompleteListener<DocumentSnapshot>() {
            @Override
            public void onComplete(@NonNull Task<DocumentSnapshot> task)
            {
                ...
                if(task.isSuccessful())
                {
                    ...
                    blocklist = task.getResult().toObject(Blocklist.class);

                    if(blocklist!= null && blocklist.getMessagelist()!= null && blocklist.getMessagelist().size()>0)
                    {
                        tinyDB.putListString("blocklist",blocklist.getMessagelist());
                    }
        ...
        startService(new Intent(getApplicationContext(),NotificationListenerExampleService.class));
        // Finally we register a receiver to tell the MainActivity when a notification has been received
       imageChangeBroadcastReceiver = new ImageChangeBroadcastReceiver();
        IntentFilter intentFilter = new IntentFilter();
        intentFilter.addAction("com.raj.notificationlistener");
